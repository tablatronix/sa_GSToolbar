...
$SA_ADMINPATH = $SITEURL.'/'.GSADMINPATH;
...
function sa_toolbar($login=null){
  ...
  GLOBAL $SATB,$SA_ADMINPATH,$SITEURL,$LANG,$USR,$datau,$SATB_MENU_ADMIN,$SATB_MENU_STATIC;
    ...
    */
    ...
    // tabs
    $tm = sa_tb_addMenu($tm,'pages','TAB_PAGES',GSADMINPATH.'pages.php');
    $tm = sa_tb_addMenu($tm,'files','TAB_FILES',GSADMINPATH.'upload.php');
    $tm = sa_tb_addMenu($tm,'theme','TAB_THEME',GSADMINPATH.'theme.php');
    $tm = sa_tb_addMenu($tm,'backups','TAB_BACKUPS',GSADMINPATH.'backups.php');
    $tm = sa_tb_addMenu($tm,'plugins','PLUGINS_NAV',GSADMINPATH.'plugins.php');

    // merge in plugin nav-tabs
    $tm = array_merge($tm,$ptabs);  
        
    $tm = sa_tb_addMenu($tm,'support','TAB_SUPPORT',GSADMINPATH.'support.php'); // custom
    $tm = sa_tb_addMenu($tm,'settings','TAB_SETTINGS',GSADMINPATH.'settings.php'); // custom
    $tm = sa_tb_addMenu($tm,'logs','LOGS',GSADMINPATH.'log.php'); // custom
    
    # satb_debugLog($ptabs);
    # satb_debugLog($tm);
    
    // default sidemenus
    $sm = sa_tb_addMenu($sm,'pages','SIDE_VIEW_PAGES',GSADMINPATH.'pages.php');
    $sm = sa_tb_addMenu($sm,'pages','SIDE_CREATE_NEW',GSADMINPATH.'edit.php');
    $sm = sa_tb_addMenu($sm,'pages','MENU_MANAGER',GSADMINPATH.'menu-manager.php');
    $sm = sa_tb_addMenu($sm,'files','FILE_MANAGEMENT',GSADMINPATH.'upload.php');
    $sm = sa_tb_addMenu($sm,'theme','SIDE_CHOOSE_THEME',GSADMINPATH.'theme.php');
    $sm = sa_tb_addMenu($sm,'theme','SIDE_EDIT_THEME',GSADMINPATH.'theme-edit.php');
    $sm = sa_tb_addMenu($sm,'theme','SIDE_COMPONENTS',GSADMINPATH.'components.php');
    $sm = sa_tb_addMenu($sm,'theme','SIDE_VIEW_SITEMAP','../sitemap.xml');
    $sm = sa_tb_addMenu($sm,'backups','SIDE_PAGE_BAK',GSADMINPATH.'backups.php');
    $sm = sa_tb_addMenu($sm,'backups','SIDE_WEB_ARCHIVES',GSADMINPATH.'archive.php');
    $sm = sa_tb_addMenu($sm,'plugins','SHOW_PLUGINS',GSADMINPATH.'plugins.php');
    // $sm = sa_tb_addMenu($sm,'plugins','anonymous_data/ANONY_TITLE',GSADMINPATH.'load.php?id=anonymous_data'); // oops, forgot this was a plugin
    $sm = sa_tb_addMenu($sm,'support','SUPPORT',GSADMINPATH.'support.php');
    $sm = sa_tb_addMenu($sm,'support','WEB_HEALTH_CHECK',GSADMINPATH.'health-check.php');
    $sm = sa_tb_addMenu($sm,'settings','GENERAL_SETTINGS',GSADMINPATH.'settings.php');
    $sm = sa_tb_addMenu($sm,'settings','SIDE_USER_PROFILE',GSADMINPATH.'settings.php#profile');
    $sm = sa_tb_addMenu($sm,'logs','VIEW_FAILED_LOGIN',GSADMINPATH.'log.php?log=failedlogins.log'); // custom
    ...
    $sm = sa_tb_addMenu($sm,'settings','TAB_LOGOUT',GSADMINPATH.'logout.php?toolbar'); // logout for convienence
    ...
    foreach($tm as $key=>$page){
        ...
        if( isset($ptabs[$key]) ){
            ...
            $tablink = GSADMINPATH.'load.php?id=' . sa_tb_array_index(sa_tb_array_index($ptabs[$key],0),'func');
...
function satb_hook_test(){
    ...
    # $SATB_MENU_STATIC['new'] = array('title'=> 'Custom New','url'=>GSADMINPATH.'load.php?id=blog&create_post');
    $SATB_MENU_STATIC['blog'] = array('title'=> '+ New Blog Post','url'=>GSADMINPATH.'load.php?id=blog&create_post');
    $SATB_MENU_STATIC['special'] = array('title'=> '+ New Special Page','url'=>GSADMINPATH.'load.php?id=i18n_specialpages&create');
